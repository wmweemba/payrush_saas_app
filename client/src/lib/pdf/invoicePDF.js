/**
 * PDF Invoice Generation for BazaBooks
...
export const generateInvoicePDF = async (invoice, profileData = {}, templateId = null) => {
  try {
    ...
    const resolvedTemplate = (templateId && staticTemplateTypes.includes(templateId))
      ? templateId
      : 'professional';
    return await generateTemplatedPDF(invoice, profileData, resolvedTemplate);
  } catch (error) {
...
export const generateDatabaseTemplatedPDF = async (invoice, profileData = {}, templateConfig) => {
  ...
  if (logoImage) {
    try {
      ...
      pdf.text('BazaBooks', textStartX, 25);
  ...
  function addTextOnlyHeader() {
    ...
    pdf.setFontSize(templateConfig.fonts.heading.size);
    pdf.setFont('helvetica', templateConfig.fonts.heading.weight);
    pdf.text('BazaBooks', templateConfig.layout.marginX, 25);
  ...
  pdf.text('Generated by BazaBooks', pageWidth / 2, footerY + 5, { align: 'center' });
