...
class InvoiceNotesService {
  ...
  async createNote(invoiceId, userId, noteData) {
    try {
      ...
      const insertData = {
        ...
        title: noteData.title ? sanitizeString(noteData.title) : null,
        content: sanitizeString(noteData.content),
        is_visible_to_customer: noteData.is_visible_to_customer || false,
        priority: noteData.priority || 'normal',
        is_system_generated: noteData.is_system_generated || false,
        system_event: noteData.system_event || null
      };

      // Validate fields
      const validNoteTypes = ['internal', 'customer_facing', 'system'];
  ...
  async updateNote(noteId, userId, updateData) {
    try {
      ...
      // Don't allow updating system-generated notes
      if (existingNote.data.is_system_generated) {
        return {
          success: false,
          error: 'System-generated notes cannot be modified',
          statusCode: 403
        };
      }
      ...
      if (updateData.title !== undefined) {
        updateFields.title = updateData.title ? sanitizeString(updateData.title) : null;
      }
      ...
      if (updateData.content !== undefined) {
        ...
        updateFields.content = sanitizeString(updateData.content);
      }
  ...
  async deleteNote(noteId, userId) {
    try {
      ...
      // Don't allow deleting system-generated notes
      if (existingNote.data.is_system_generated) {
        return {
          success: false,
          error: 'System-generated notes cannot be deleted',
          statusCode: 403
        };
      }
  ...
  /**
   * Create a system note (automatic note generated by the system)
   */
  async createSystemNote(invoiceId, userId, systemEvent, content, title = null) {
    try {
      const noteData = {
        content: content,
        title: title,
      ...
      return await this.createNote(invoiceId, userId, noteData);
    } catch (error) {
      console.error('Error in createSystemNote:', error);
      return {
        success: false,
        error: 'Internal server error',
        statusCode: 500
    ...
    try {
      ...
      const summaryData = {
        ...
        by_type: {
          ...
          customer_facing: summary.filter(n => n.note_type === 'customer_facing').length,
